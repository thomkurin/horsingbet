...
if (isset($_GET['event_id'])) {
    ...
    // Consulte o banco de dados para obter as categorias do evento com o ID correspondente
    $sql = "SELECT * FROM categorias WHERE event_id = $eventId";
    ...
    if ($result === false) {
        ...
        if ($result->num_rows > 0) {
            // Loop para exibir as categorias encontradas
            while ($row = $result->fetch_assoc()) {
                echo "<div class='categoria-box'>";
                echo "<a href='largada.php?event_id=" . $row['event_id'] . "&category_id=" . $row['id'] . "' class='categoria'>";
                echo "<h3>" . $row['name'] . "</h3>";
                echo "</a>";
                echo "</div>";
            }
        } else {
            echo "Nenhuma categoria encontrada para o evento selecionado.";
        }
    ...
    // ID do evento não fornecido na URL
